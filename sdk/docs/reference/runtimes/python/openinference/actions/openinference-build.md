...
- [**Task**](#task){ .list-card-link } - Configure the OpenInference build Task.

- [**Run**](#run){ .list-card-link } - Build the OpenInference function image.
...
??? example "build"
    ...
    Build the OpenInference function image.
...
The OpenInference build Task does not add runtime-specific methods.

## Run

??? example "Create a run"

    Build the OpenInference function image and return the resulting `Run` entity.
