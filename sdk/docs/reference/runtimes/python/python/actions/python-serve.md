...
- [**Task**](#task){ .list-card-link } - Configure the Python serve Task.

- [**Run**](#run){ .list-card-link } - Deploy the Python function as a service.
...
The serve action supports Python handlers that respond to HTTP requests.
...
The Python serve Task does not add runtime-specific methods.

## Run

??? example "Create a run"

    Deploy the Python function as an HTTP service and return the resulting `Run` entity.
...
??? example "invoke"
    ...
    Invoke the Python service endpoint with an HTTP request.
