...
- [**Function**](#function){ .list-card-link } - Create a vLLM speech Function.

- [**Task**](#task){ .list-card-link } - Configure the vLLM speech Task.

- [**Run**](#run){ .list-card-link } - Serve the vLLM speech model.
...
The vllmserve-speech function kind supports serving speech models with vLLM for speech processing workloads.
...
The vLLM speech Function does not add runtime-specific methods.
...
The vLLM speech Task does not add runtime-specific methods.
...
??? example "invoke"
    ...
    Invoke the vLLM speech model serving run with input data.
...
??? example "list_endpoints"
    ...
    ::: digitalhub_runtime_modelserve.entities.run.vllmservespeech_run.entity.RunVllmservespeechRun.list_endpoints
        options:
            heading_level: 6
            show_signature: false
            show_docstring_description: true
