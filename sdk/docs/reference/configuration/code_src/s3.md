...
Use a `zip+s3://`, `zip+s3a://` or `zip+s3n://` URI to point the runtime at a ZIP archive stored on S3 (or S3-compatible object storage).
...
- `zip+s3a://bucket/path/to/archive.zip`
- `zip+s3n://bucket/path/to/archive.zip`
