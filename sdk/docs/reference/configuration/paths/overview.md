...
The SDK supports several storage path schemes. Choose the scheme that matches where your files live and follow the scheme-specific rules below.
...
- [Hugging Face paths](hf.md) — model repositories downloaded from the Hugging Face Hub.
- [SQL paths](sql.md) — database tables (Postgres-compatible).

The accepted scheme aliases are:
...
- Non-local paths must include a scheme (for example `s3://`, `https://`, `hf://`, `sql://`).
...
The `zip+http(s)` and `zip+s3` prefixes are for [code sources](../code_src/overview.md), not for data paths.
