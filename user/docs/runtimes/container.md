...
The **Container runtime** allows you to create deployments, jobs and services on Kubernetes, starting from arbitrary user-defined container images. A container function may be run:

- as a **Job** (``action="job"``), representing a single piece of work (e.g., model training, data processing).
...
To customize the image to a specific context, it is possible to perform a ``build`` operation on the function. This operation creates a container image starting from the base image and an optional list of additional instructions. Next time the Job or Service starts, this prebuilt container image will be used for execution.
